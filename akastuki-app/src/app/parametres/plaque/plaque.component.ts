...
export class PlaqueComponent implements OnInit {
  ...
  rechercher(){
    this.page.index = 0
    this.getPlaque(this.searchData);
  }
  ...
  getCluster(param?: any) {
    ...
    const data = {
      serviceLibelle: `Consultation de la liste des cluster`,
      ndCode: '',
      customerCode: '',
    ...
    this.restApi.executeRessources(methode, data).subscribe((res: any) => {
      if (res && !res.hasError) {
        this.listeCluster = res.items ? res.items : [];
      } else {
  ...
  getPlaque(param?: any) {
    ...
    const data = {
      serviceLibelle: `Consultation de la liste des plaque`,
      index: this.page.index,
      size: this.page.size,
      data: {
        libelle: param && param.libelle ? param.libelle : '',
        clusterId: param && param.clusterId ? param.clusterId : ''
      },
      orderBy: 'desc',
      takeAll: true
    };
    ...
    this.restApi.executeRessources(methode, data).subscribe((res: any) => {
      if (res && !res.hasError) {
        this.listePlaque = res && res.items ? res.items : [];
  ...
  openModal(param?: any){
    const dialogRef = this.dialog.open(ModalPlaqueComponent, {
      width: '600px',
      disableClose: true,
      data: param
    });
