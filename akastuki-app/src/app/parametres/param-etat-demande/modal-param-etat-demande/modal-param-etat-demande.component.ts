...
declare var $: any;
@Component({
  selector: 'app-modal-param-etat-demande',
  templateUrl: './modal-param-etat-demande.component.html',
  styleUrls: ['./modal-param-etat-demande.component.scss']
})
export class ModalParamEtatDemandeComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalParamEtatDemandeComponent>,
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'paramEtatDemande/update';
    } else {
        methode = 'paramEtatDemande/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de params état demande`,
