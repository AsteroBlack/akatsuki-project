...
declare var $: any;
@Component({
  selector: 'app-modal-arrondissement',
  templateUrl: './modal-arrondissement.component.html',
  styleUrls: ['./modal-arrondissement.component.scss']
})
export class ModalArrondissementComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  listeVilles : any[] = [];
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalArrondissementComponent>,
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'arrondissement/update';
    } else {
        methode = 'arrondissement/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} d'arrondissement`,
