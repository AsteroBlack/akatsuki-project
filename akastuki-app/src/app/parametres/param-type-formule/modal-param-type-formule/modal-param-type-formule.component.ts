...
declare var $: any;
@Component({
  selector: 'app-modal-param-type-formule',
  templateUrl: './modal-param-type-formule.component.html',
  styleUrls: ['./modal-param-type-formule.component.scss']
})
export class ModalParamTypeFormuleComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalParamTypeFormuleComponent>,
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'paramTypeFormule/update';
    } else {
        methode = 'paramTypeFormule/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      // user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de params type Formule`,
