...
declare var $: any;
@Component({
  selector: 'app-modal-plaque-quartier',
  templateUrl: './modal-plaque-quartier.component.html',
  styleUrls: ['./modal-plaque-quartier.component.scss']
})
export class ModalPlaqueQuartierComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalPlaqueQuartierComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any) {
      this.formModal = new FormGroup({
        id: new FormControl(null),
        libelle: new FormControl(null, [Validators.required]),
        quartierId: new FormControl(null, [Validators.required]),
        plaqueId: new FormControl(null, [Validators.required]),
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'plaqueQuartier/update';
    } else {
        methode = 'plaqueQuartier/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de plaqueQuartier`,
