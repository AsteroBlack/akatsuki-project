...
declare var $: any;
@Component({
  selector: 'app-modal-ville',
  templateUrl: './modal-ville.component.html',
  styleUrls: ['./modal-ville.component.scss']
})
export class ModalVilleComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalVilleComponent>,
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'ville/update';
    } else {
        methode = 'ville/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de params ville`,
