...
declare var $: any;
@Component({
  selector: 'app-modal-quartier',
  templateUrl: './modal-quartier.component.html',
  styleUrls: ['./modal-quartier.component.scss']
})
export class ModalQuartierComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  listSecteur : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalQuartierComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any) {
      this.formModal = new FormGroup({
        id: new FormControl(null),
        libelle: new FormControl(null, [Validators.required]),
        secteurId: new FormControl(null, [Validators.required]),
  ...
  getSecteur(param?: any) {
    ...
    this.restApiClient.executeRessources(methode, data).subscribe((res: any) => {
      console.log('res', res);
      if (res && !res.hasError) {
        this.listSecteur = res.items ? res.items: [];
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'quartier/update';
    } else {
        methode = 'quartier/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de quartier`,
