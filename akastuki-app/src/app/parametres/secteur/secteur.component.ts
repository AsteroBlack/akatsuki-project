...
import { ModalSecteurComponent } from './modal-secteur/modal-secteur.component';
...
export class SecteurComponent implements OnInit {
  ...
  rechercher(){
    this.getSecteur(this.searchData);
  }
  ...
  getSecteur(param?: any) {
    ...
    const data = {
      serviceLibelle: `Consultation de la liste des secteurs`,
    ...
    this.restApi.executeRessources(methode, data).subscribe((res: any) => {
      if (res && !res.hasError) {
        this.listeSecteur = res && res.items ? res.items : [];
        this.page.total = res.count;
        // this.activedId = 'panel-3';
      } else {
        this.toastService.error('Opération échouée', res && res.status && res.status.message ? res.status.message : 'impossible de recupérer les secteurs');
      }
    }, (err: any) => {
  ...
  openModal(param?: any){
    const dialogRef = this.dialog.open(ModalSecteurComponent, {
      width: '600px',
      disableClose: true,
      data: param
    });
