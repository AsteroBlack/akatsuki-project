...
declare var $: any;
@Component({
  selector: 'app-modal-param-tiroir-distrib',
  templateUrl: './modal-param-tiroir-distrib.component.html',
  styleUrls: ['./modal-param-tiroir-distrib.component.scss']
})
export class ModalParamTiroirDistribComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalParamTiroirDistribComponent>,
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'paramTiroirDistrib/update';
    } else {
        methode = 'paramTiroirDistrib/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de params tiroir distrib`,
