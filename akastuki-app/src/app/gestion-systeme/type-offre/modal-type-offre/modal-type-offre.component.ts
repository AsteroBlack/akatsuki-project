...
declare var $: any;
@Component({
  selector: 'app-modal-type-offre',
  templateUrl: './modal-type-offre.component.html',
  styleUrls: ['./modal-type-offre.component.scss']
})
export class ModalTypeOffreComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  jsTreeElt: any;
  listTypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalTypeOffreComponent>,
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'typeOffre/update';
    } else {
        methode = 'typeOffre/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
    ...
    console.log('data typeOffre', JSON.stringify(data));
    this.restApiClient.execute(methode, data).subscribe((res: any) => {
      console.log('res', res);
      if (res && !res.hasError) {
