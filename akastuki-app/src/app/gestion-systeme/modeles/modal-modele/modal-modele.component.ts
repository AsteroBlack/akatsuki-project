...
export class ModalModeleComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  jsTreeElt: any;
  // listetypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalModeleComponent>,
    @Inject(MAT_DIALOG_DATA) public data: any) {
      this.formModal = new FormGroup({
        id: new FormControl(null),
        libelle: new FormControl(null, [Validators.required]),
        coefMac: new FormControl(null, [Validators.required]),
    ...
    ngAfterContentInit(){
      ...
      if (this.data) {
        ...
        this.formModal.controls.coefMac.patchValue(this.data.coefficientModele.coefMac);
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'modele/update';
    } else {
        methode = 'modele/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de modele`,
