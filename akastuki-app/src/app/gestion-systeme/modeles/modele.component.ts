...
export class ModeleComponent implements OnInit {
  ...
  openModal(param?: any){
    const dialogRef = this.dialog.open(ModalModeleComponent, {
      width: '600px',
      disableClose: true,
      data: param
    });
  ...
  testMenu(action:string){
    return this.userService.checkMenuExiste('GESTION_DES_OFFRES','MODELE',action)
  }
}
