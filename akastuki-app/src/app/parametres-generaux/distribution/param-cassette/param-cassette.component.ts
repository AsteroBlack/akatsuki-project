...
import { ModalParamCassetteComponent } from './modal-param-cassette/modal-param-cassette.component';
import { CommunicationService } from 'src/app/shared/services/communication.service';
import { Router } from '@angular/router';
import { SubMenus } from '../distribution-submenus';

@Component({
  selector: 'app-param-cassette',
  templateUrl: './param-cassette.component.html',
  styleUrls: ['./param-cassette.component.scss'],
  providers:[SubMenus],
})
export class ParamCassetteComponent implements OnInit {
  page: any = {
    index: 0,
    size: 10,
    total: 0
  };
  ParamsCassetteArrayColumn = [
    'num',
    'libelle',
    'options',
    // 'datasRole'
  ...
  getParamsCassette() {
    // const methode = 'costomer-back-end-api-1.0/paramCassette/getByCriteria';
    // const methode = 'api-customer-care-fixe/getCassetteCriteria';
    const methode = 'paramCassette/getByCriteria';
    const data = {
      serviceLibelle: `Consultation de la liste des params cassette`,
    ...
    this.restApi.executeRessources(methode, data).subscribe((res: any) => {
      if (res && !res.hasError) {
        this.listeParamsCassette = res && res.items ? res.items : [];
        this.page.total = res.count;
        // this.activedId = 'panel-3';
      } else {
        this.toastService.error('Opération échouée', res && res.status && res.status.message ? res.status.message : 'impossible de recupérer les params cassettes');
      }
    }, (err: any) => {
  ...
  openModal(param?: any){
    const dialogRef = this.dialog.open(ModalParamCassetteComponent, {
      width: '600px',
      disableClose: true,
      data: param
    });
  ...
  deleteAction(param?: any) {
    const methode = 'paramCassette/delete';
    const data = {
