...
declare var $: any;
@Component({
  selector: 'app-modal-param-pd-type',
  templateUrl: './modal-param-pd-type.component.html',
  styleUrls: ['./modal-param-pd-type.component.scss']
})
export class ModalParamPdTypeComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalParamPdTypeComponent>,
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'paramPdType/update';
    } else {
        methode = 'paramPdType/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de params pd type`,
