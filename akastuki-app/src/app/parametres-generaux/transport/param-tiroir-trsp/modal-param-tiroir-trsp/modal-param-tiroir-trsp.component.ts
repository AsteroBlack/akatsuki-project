...
declare var $: any;
@Component({
  selector: 'app-modal-param-tiroir-trsp',
  templateUrl: './modal-param-tiroir-trsp.component.html',
  styleUrls: ['./modal-param-tiroir-trsp.component.scss']
})
export class ModalParamTiroirTrspComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalParamTiroirTrspComponent>,
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'paramTiroirTrsp/update';
    } else {
        methode = 'paramTiroirTrsp/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de params tiroir Trsp`,
