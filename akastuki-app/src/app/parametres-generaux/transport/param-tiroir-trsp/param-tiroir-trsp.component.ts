...
import { ModalParamTiroirTrspComponent } from './modal-param-tiroir-trsp/modal-param-tiroir-trsp.component';
import { Router } from '@angular/router';
import { CommunicationService } from 'src/app/shared/services/communication.service';
import { SubMenus } from '../transport-submenus';

@Component({
  selector: 'app-param-tiroir-trsp',
  templateUrl: './param-tiroir-trsp.component.html',
  styleUrls: ['./param-tiroir-trsp.component.scss'],
  providers:[SubMenus],

})
export class ParamTiroirTrspComponent implements OnInit {
  page: any = {
    index: 0,
    size: 10,
    total: 0
  };
  ParamsTiroirTrspArrayColumn = [
    'num',
    'libelle',
    'options',
    // 'datasRole'
  ...
  getParamsTiroirTrsp() {
    ...
    const data = {
      serviceLibelle: `Consultation de la liste des params tiroir Trsp`,
    ...
    this.restApi.executeRessources(methode, data).subscribe((res: any) => {
      if (res && !res.hasError) {
        this.listeParamsTiroirTrsp = res && res.items ? res.items : [];
        this.page.total = res.count;
        // this.activedId = 'panel-3';
      } else {
        this.toastService.error('Opération échouée', res && res.status && res.status.message ? res.status.message : 'impossible de recupérer les params tiroir Trsp');
      }
    }, (err: any) => {
  ...
  openModal(param?: any){
    const dialogRef = this.dialog.open(ModalParamTiroirTrspComponent, {
      width: '600px',
      disableClose: true,
      data: param
    });
  ...
  deleteAction(param?: any) {
    const methode = 'paramTiroirTrsp/delete';
    const data = {
