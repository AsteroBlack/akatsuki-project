...
@Component({
  selector: 'app-param-coupleur',
  templateUrl: './param-coupleur.component.html',
  styleUrls: ['./param-coupleur.component.scss']
})
export class ParamCoupleurComponent implements OnInit {
  page: any = {
    index: 0,
    size: 10,
    total: 0
  };
  ParamsCoupleurArrayColumn = [
    'num',
    'libelle',
    'options',
    // 'datasRole'
  ...
  getParamsCoupleur() {
    ...
    this.restApi.executeRessources(methode, data).subscribe((res: any) => {
      if (res && !res.hasError) {
        this.listeParamsCoupleur = res && res.items ? res.items : [];
  ...
  openModal(param?: any){
    const dialogRef = this.dialog.open(ModalParamCoupleurComponent, {
      width: '600px',
      disableClose: true,
      data: param
    });
  ...
  deleteAction(param?: any) {
    const methode = 'paramCoupleur/delete';
    const data = {
