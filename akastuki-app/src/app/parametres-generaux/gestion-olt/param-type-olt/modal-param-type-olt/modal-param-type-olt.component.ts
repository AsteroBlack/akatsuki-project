...
declare var $: any;
@Component({
  selector: 'app-modal-param-type-olt',
  templateUrl: './modal-param-type-olt.component.html',
  styleUrls: ['./modal-param-type-olt.component.scss']
})
export class ModalParamTypeOltComponent implements OnInit, AfterContentInit {
  formModal: FormGroup;
  nomFichier: string = '';
  listTypeOffre : any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ModalParamTypeOltComponent>,
  ...
  onSubmit(param: any){
    ...
    if(this.formModal.value.id){
        methode = 'paramTypeOlt/update';
    } else {
        methode = 'paramTypeOlt/create';
    }
    const data = {
      ndCode: '',
      customerCode: '',
      // user: /*this.userGetData ? this.userGetData.id : */ '1118',
      serviceLibelle: `${this.formModal.value.id ? 'Mise à jours' : 'Création'} de params type olt`,
