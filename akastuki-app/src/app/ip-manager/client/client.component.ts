...
export class ClientComponent implements OnInit {
  ...
  rechercher(){
    this.page.index = 0
    this.getClient(this.searchData);
  }
  ...
  getClient(param?: any) {
    ...
    this.restApi.executeIpManager(methode, data).subscribe((res: any) => {
      if (res && !res.hasError) {
        this.listeClient = res && res.items ? res.items : [];
        this.page.total = res.count;
        // this.activedId = 'panel-3';
      } else {
        this.toastService.error('Opération échouée', res && res.status && res.status.message ? res.status.message : 'impossible de recupérer les clients');
      }
    }, (err: any) => {
