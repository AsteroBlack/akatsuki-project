...
export class ModulesComponent implements OnInit {
  ...
  getModules() {
    ...
    const data = {
      serviceLibelle: `Consultation de la liste des module`,
      ndCode: '',
      customerCode: '',
    ...
    this.restApi.execute(methode, data).subscribe((res: any) => {
      if (res && !res.hasError) {
        this.listeModules = res && res.items ? res.items : [];
  ...
  openModal(param?: any){
    const dialogRef = this.dialog.open(ModalModulesComponent, {
      width: '600px',
      disableClose: true,
      data: param
    });
  ...
  testMenu(action:string){
    return this.userService.checkMenuExiste('ADMINISTRATION','MODULES',action)
  }
}
